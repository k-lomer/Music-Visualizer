...
MovingWaveLayer::MovingWaveLayer(int num_waves, orientation wave_orientation, double wave_movement, int width, int height, int wave_amplitude, SDL_Color wave_color)
	...
	for (int i = 0; i < num_waves + 1; ++i) {
		...
		if ( wave_movement > 0) {
			wave_positions.push_back(-double(length) * double(i) / double(num_waves));
		}
		else if (wave_movement < 0) {
			wave_positions.push_back(double(length) + double(length) * double(i) / double(num_waves));
		}
		else { // movement is zero so fix waves in place
...
void MovingWaveLayer::draw(SDL_Renderer * renderer, const packet & packet_buffer) {
	...
	for (double pos : wave_positions) {
		if (wave_orientation == Horizontal) {
			draw_wave(renderer, packet_buffer, SDL_Point{0, int(pos)}, SDL_Point{window_width, int(pos) }, amplitude, color);
		}
		else {
			draw_wave(renderer, packet_buffer, SDL_Point{ int(pos), 0}, SDL_Point{ int(pos), window_height }, amplitude, color);
		}
	}
