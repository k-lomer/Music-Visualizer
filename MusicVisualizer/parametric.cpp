...
SDL_FPoint ParametricEquation2d::operator () (double t) {
    return SDL_FPoint{ float(scale * cos(x_a * t)), float(scale * sin(y_b * t)) };
}

std::vector<SDL_FPoint> ParametricEquation2d::gen_even_points(int num_points, double t_offset, bool join_points) {
    double step = 2.0 * M_PI / double(num_points);
